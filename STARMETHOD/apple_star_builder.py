...
def apple_star_builder():
    """
    Enhanced STAR method story builder specifically for Apple interviews.
    Incorporates Apple competency frameworks and provides tailored guidance.
    """
    # Initialize competency database from PDFs
    print("Loading Apple competency frameworks from PDF files...")
    apple_competency_data = load_apple_competencies_from_pdfs()
    
    # Add this line for debugging
    visualize_competency_data(apple_competency_data)
    
    apple_competencies = apple_competency_data['roles_competencies']
    competency_details = apple_competency_data['competency_details']
    
    print("\n🍎 Welcome to the Apple Interview STAR Story Builder!")
    print("This program helps structure your answers using the STAR method")
    print("while focusing on Apple's key competencies.")
    print("\n**The STAR Method:**")
    ...
    # Select role
    print("\nFirst, let's identify the Apple role you're interviewing for:")
    available_roles = list(apple_competencies.keys())
    ...
    role_competencies = apple_competencies[selected_role]
    ...
    while True:
        ...
        print(f"\nAnalyzing: \"{question}\"")
        print("\nThis question likely targets these Apple competencies:")
        ...
        print("\n" + "="*50)
        print("📱 YOUR ENHANCED APPLE INTERVIEW RESPONSE:")
        print(f"\nQuestion: {question}")
...
def get_competency_situation_prompt(competency, competency_details=None):
    ...
    if competency_details and competency in competency_details and competency_details[competency]['situation']:
        return f"Apple's guidance for {competency} situations:\n{competency_details[competency]['situation']}"
    
    # Updated prompts for Apple competencies
    prompts = {
...
def get_competency_task_prompt(competency, competency_details=None):
    ...
    if competency_details and competency in competency_details and competency_details[competency]['task']:
        return f"Apple's guidance for {competency} tasks:\n{competency_details[competency]['task']}"
    
    # Fall back to existing prompts if no details found
    prompts = {
...
def get_competency_action_prompt(competency, competency_details=None):
    ...
    if competency_details and competency in competency_details and competency_details[competency]['action']:
        return f"Apple's guidance for {competency} actions:\n{competency_details[competency]['action']}"
    
    # Maintain your existing comprehensive prompt dictionary
    # Just ensure the keys exactly match the competency names from the PDF
    prompts = {
    # Just ensure the keys exactly match the competency names from the PDF
    prompts = {
...
def get_competency_result_prompt(competency, competency_details=None):
    ...
    if competency_details and competency in competency_details and competency_details[competency]['result']:
        return f"Apple's guidance for {competency} results:\n{competency_details[competency]['result']}"
    
    # Complete prompts for different competencies
    prompts = {
...
def enhance_text(text, competency, section, competency_details=None):
    ...
    if word_count_val < 20:
        ...
        # Add guidance from competency details if available
        if competency_details and competency in competency_details and competency_details[competency][section]:
            ...
            note += f" Apple example: {examples[:100]}...]"
        else:
            note += "]"
...
def save_enhanced_story(story, question, competency, filename="apple_star_stories.txt"):
...
def load_apple_competencies_from_pdfs():
    ...
    """
    # Try to extract from PDF first, but with better error handling
    try:
        ...
        if os.path.exists(roles_pdf_path):
            try:
                with open(roles_pdf_path, 'rb') as f:
                    ...
                    # Adapt these patterns based on the actual PDF format
                ...
                if file.endswith('.pdf') and file != "Competencies.pdf":
                    try:
                        ...
                        with open(pdf_path, 'rb') as f:
                            ...
                            # This is a simplified pattern match that would need customization
        ...
        print(f"Error parsing competency PDF: {e}")
        print("Falling back to official competency data")
        return load_apple_competencies_fallback()

def load_apple_competencies_fallback():
    """Fallback function that returns accurate Apple competencies if PDF parsing fails."""
    print("Using official Apple competency data.")
    return {
        'roles_competencies': {
